...
.autopilot/
  ...
  archive/
    {timestamp}/         — snapshot of prior session when user chose "Start fresh"
  FINAL.md               — written when loop terminates
```

### Resume vs Fresh

`/autopilot` inspects `.autopilot/STATUS.md` at launch. If a prior session exists and is not `completed`, the user is prompted to **Resume** (skip kickoff, continue from next iteration) or **Start fresh** (archive prior state to `archive/{timestamp}/`). HISTORY.md is passed as context into Phase 1 and Phase 2 so the Proposer and Challenger do not re-propose already-completed work. See `skills/autopilot-loop/SKILL.md` → "Resume Flow".
